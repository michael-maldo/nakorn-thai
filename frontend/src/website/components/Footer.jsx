      ...
      <div className="footer-main page-width">
        ...
        {footerGroups.map((group) => (
          <nav className="footer-links" aria-label={group.title} key={group.title}>
            <h3>{group.title}</h3>
            {group.links.map((link) => <a key={link} href={link === 'Book a Table' ? '#/reservations' : ['Functions', 'Private Dining'].includes(link) ? '#/functions' : `#${link.toLowerCase().replaceAll(' ', '-')}`}>{link}</a>)}
          </nav>
        ))}
