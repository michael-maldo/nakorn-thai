...
export default function MenuItemEditor({ item, route, state }) {
  ...
  async function save(event) {
    ...
    if (!item && result.committed) state.navigate(menuAdminHref('items', result.result.id, 'options'));
  }
  return <>
    {item && <><div className="menu-admin-meta"><StatusBadge value={item.status} /><span>{item.available ? 'Enabled for ordering' : 'Unavailable for ordering'}</span></div><nav className="menu-admin-subnav" aria-label="Item sections">{itemSections.map(section => <a key={section} aria-current={route.section === section ? 'page' : undefined} href={menuAdminHref('items', item.id, section)}>{section === 'options' ? 'Options & extras' : section === 'pricing' ? 'Pricing / variations' : section[0].toUpperCase() + section.slice(1)}</a>)}</nav></>}
    {route.section === 'options' ? <MenuItemOptionEditor item={item} authorization={state.authorization} /> : route.section === 'images' ? <div className="menu-admin-form"><h2>Item image</h2><MenuImageEditor item={item} authorization={state.authorization} csrf={true} disabled={state.disabled} onBusy={setImageBusy} onSaved={async () => { await state.reload(); }} /><p>{imageBusy ? 'Saving image…' : 'Image changes save independently of item details.'}</p></div> : <form className="menu-admin-form" onSubmit={save}>
      <fieldset disabled={state.disabled}><legend>{route.section === 'pricing' ? 'Pricing & variations' : route.section === 'collections' ? 'Collection membership' : 'Item overview'}</legend>
        ...
        </div><p className="menu-admin-hint">Changing the name or description clears dietary verification and requires allergen review. Food declarations are not editable in this interface.</p>{!item && <p>Create the item first, then configure reusable options and item-specific prices in the next step. You can also add base pricing, collections and an image.</p>}</>}
        ...
        <FormActions busy={state.busy} disabled={state.disabled} dirty={form.dirty} onCancel={() => { form.reset(); state.clearFeedback(); }} label={item ? 'Save changes' : 'Create item & configure options'} />
      </fieldset>
    </form>}
